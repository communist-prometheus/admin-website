...
/** The latest magazine issue for a language, or none. */
...
export type MagazineFetcher = (lang: Lang) => Promise<Article | undefined>
...
/**
 * Build a per-language magazine fetcher bound to the given base URL.
 * Reads `/<lang>/magazine.xml` and returns the newest published
 ...
 */
export const createMagazineFetcher = (d: Deps = {}): MagazineFetcher => {
  const base = d.base ?? FALLBACK_BASE
  const doFetch = d.fetch ?? globalThis.fetch.bind(globalThis)
  return async lang => {
    const body = await safeFetch(doFetch, `${base}/${lang}/magazine.xml`)
