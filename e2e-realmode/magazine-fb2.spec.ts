...
test('magazine FB2 upload commits sources to assets/<slug>.<lang>.fb2', async ({
  page,
}) => {
  test.setTimeout(180_000)
  await bootRealmode(page, 'magazine-fb2')
  await visit(page, `/content/magazine/edit/${SLUG}`, SLOW)
  /* Magazine edit doesn't mount a Markdown body editor — the
   ...
   * lang on `/content/magazine/edit/...` is EN, so the expected
   * path is `<slug>.en.fb2`. */
  await page
...
test('uploaded FB2 is stripped of converter metadata before commit', async ({
  ...
  test.setTimeout(180_000)
  await bootRealmode(page, 'magazine-fb2-sanitize')
  await visit(page, `/content/magazine/edit/${SLUG}`, SLOW)
  await expectVisible(
    page,
    page.locator('[data-testid="magazine-source-uploads"]'),
  ...
  const remote =
    (await readFile(`magazine/${SLUG}/assets/${SLUG}.en.fb2`)) ?? ''
