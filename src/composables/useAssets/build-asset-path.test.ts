...
describe('buildAssetPath', () => {
  it('builds full path for an asset', () => {
    ...
    expect(result).toBe('src/content/blog/my-post/assets/hero.svg')
  })

  it('handles slugs with hyphens', () => {
    const result = buildAssetPath('a-b-c', 'img.png')
    expect(result).toBe('src/content/blog/a-b-c/assets/img.png')
  })
})

describe('buildAssetsPrefix', () => {
  it('builds assets directory prefix', () => {
    expect(buildAssetsPrefix('my-post')).toBe(
      'src/content/blog/my-post/assets'
    )
  })
})
