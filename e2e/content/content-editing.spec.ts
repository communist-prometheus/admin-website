...
test.describe('Content Editing', () => {
  ...
  test('should show editor when clicking on content item', async ({ page }) => {
  ...
  test('should load content into editor when item is selected', async ({ page }) => {
    const firstItem = page.locator('[data-testid="content-item"]').first()
    await firstItem.click()
  ...
  test('should send save request when save button is clicked', async ({ page }) => {
    const firstItem = page.locator('[data-testid="content-item"]').first()
    await firstItem.click()
    await page.waitForTimeout(1000)
    
    const savePromise = page.waitForResponse(resp => 
      resp.url().includes('/api/github/content') && resp.request().method() === 'POST'
    )
  ...
  test('should clear editor when switching to different item', async ({ page }) => {
  ...
  test('should preserve unsaved changes when navigating away and back', async ({ page }) => {
    const firstItem = page.locator('[data-testid="content-item"]').first()
    await firstItem.click()
    await page.waitForTimeout(1000)
